...
//
func (check *Checker) ident(x *operand, e *Ident, def *Named, path []*TypeName) {
	x.mode = invalid
	x.expr = e
...
// in the path.
//
func (check *Checker) typExpr(e Expr, def *Named, path []*TypeName) (T Type) {
	if trace {
...
func (check *Checker) funcType(sig *Signature, recvPar *FieldList, ftyp *FuncType) {
...
//
func (check *Checker) typExprInternal(e Expr, def *Named, path []*TypeName) Type {
	switch e := e.(type) {
	case *BadExpr:
...
//
func (check *Checker) typOrNil(e Expr) Type {
	var x operand
...
func (check *Checker) collectParams(scope *Scope, list *FieldList, variadicOk bool) (params []*Var, variadic bool) {
	if list == nil {
		return
	}
...
func (check *Checker) interfaceType(ityp *Interface, iface *InterfaceType, def *Named, path []*TypeName) {
...
func (check *Checker) structType(styp *Struct, e *StructType, path []*TypeName) {
	list := e.Fields
	if list == nil {
		return
	}
	...
	var typ Type
	var tag string
	add := func(ident *Ident, embedded bool, pos token.Pos) {
		if tag != "" && tags == nil {
	...
	// (issue #25627).
	addInvalid := func(ident *Ident, pos token.Pos) {
		typ = Typ[Invalid]
		tag = ""
		add(ident, true, pos)
	}
...
func embeddedFieldIdent(e Expr) *Ident {
	switch e := e.(type) {
	case *Ident:
		return e
	case *StarExpr:
		// *T is valid, but **T is not
		if _, ok := e.X.(*StarExpr); !ok {
