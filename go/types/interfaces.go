...
func (check *Checker) infoFromTypeLit(scope *Scope, iface *InterfaceType, tname *TypeName, path []*TypeName) (info *ifaceInfo) {
	assert(iface != nil)
...
func (check *Checker) infoFromTypeName(scope *Scope, name *Ident, path []*TypeName) *ifaceInfo {
...
func (check *Checker) infoFromQualifiedTypeName(scope *Scope, qname *SelectorExpr) *ifaceInfo {
	// see also Checker.selector
	name, _ := qname.X.(*Ident)
	if name == nil {
		return nil
	}
