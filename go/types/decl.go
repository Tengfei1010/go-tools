...
func (check *Checker) declare(scope *Scope, id *Ident, obj Object, pos token.Pos) {
		...
		case *TypeName:
			...
			if fixFor26390 {
				...
				if d := check.objMap[obj]; d != nil && d.alias && obj.typ == Typ[Invalid] {
					...
					if tname := check.resolveBaseTypeName(NewIdent(obj.name)); tname != nil && tname.typ != nil {
						obj.typ = tname.typ
						break
					}
				}
...
func (check *Checker) constDecl(obj *Const, typ, init Expr) {
	assert(obj.typ == nil)
...
func (check *Checker) varDecl(obj *Var, lhs []*Var, typ, init Expr) {
	...
	check.initVars(lhs, []Expr{init}, token.NoPos)
}
...
func (check *Checker) typeDecl(obj *TypeName, typ Expr, def *Named, path []*TypeName, alias bool) {
	assert(obj.typ == nil)
...
func (check *Checker) declStmt(decl Decl) {
	pkg := check.pkg
	...
	case *GenDecl:
		var last *ValueSpec // last ValueSpec with type or init exprs seen
			...
			case *ValueSpec:
				...
				case token.CONST:
					...
					case s.Type != nil || len(s.Values) > 0:
						last = s
					case last == nil:
						last = new(ValueSpec) // make sure last exists
					}
	...
	default:
		check.invalidAST(d.Pos(), "unknown Decl node %T", d)
	}
}
