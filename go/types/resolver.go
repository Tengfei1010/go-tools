...
func (check *Checker) arityMatch(s, init *ValueSpec) {
	l := len(s.Names)
	r := len(s.Values)
	if init != nil {
		r = len(init.Values)
...
func (check *Checker) collectObjects() {
	...
	for fileNo, file := range check.files {
		...
		// Use the actual source file extent rather than *File extent since the
		// latter doesn't include comments which appear at the start or end of the file.
		// Be conservative and use the *File extent if we don't have a *token.File.
		...
		for _, decl := range file.Decls {
			...
			case *GenDecl:
				...
				for iota, spec := range d.Specs {
					...
					default:
						check.invalidAST(s.Pos(), "unknown Spec node %T", s)
					}
				}
	...
	for _, f := range methods {
		...
		if list := fdecl.Recv.List; len(list) > 0 {
			...
			if ptr, _ := typ.(*StarExpr); ptr != nil {
				typ = unparen(ptr.X)
			}
			if base, _ := typ.(*Ident); base != nil {
...
// type name; or nil.
func (check *Checker) resolveBaseTypeName(name *Ident) *TypeName {
	var path []*TypeName
	for {
