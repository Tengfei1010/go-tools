...
func (check *Checker) multipleDefaults(list []Stmt) {
	var first Stmt
	for _, s := range list {
		var d Stmt
		switch c := s.(type) {
...
func (check *Checker) openScope(s Stmt, comment string) {
...
func (check *Checker) caseTypes(x *operand, xtyp *Interface, types []Expr, seen map[Type]token.Pos) (T Type) {
L:
	for _, e := range types {
		T = check.typOrNil(e)
		if T == Typ[Invalid] {
...
func (check *Checker) stmt(ctxt stmtContext, s Stmt) {
	...
	case *IncDecStmt:
		...
		Y := &BasicLit{ValuePos: s.X.Pos(), Kind: token.INT, Value: "1"} // use x's position
	...
	case *SwitchStmt:
		...
		if s.Tag != nil {
			...
			x.expr = &Ident{NamePos: s.Body.Lbrace, Name: "true"}
		}
	...
	case *TypeSwitchStmt:
		...
		//
		var lhs *Ident // lhs identifier or nil
		var rhs Expr
	...
	case *SelectStmt:
		...
		for _, s := range s.Body.List {
			...
			if rhs != nil {
				if x, _ := unparen(rhs).(*UnaryExpr); x != nil && x.Op == token.ARROW {
					valid = true
				}
			}
	...
	case *ForStmt:
		...
		if s, _ := s.Post.(*AssignStmt); s != nil && s.Tok == token.DEFINE {
