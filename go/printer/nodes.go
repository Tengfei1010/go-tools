...
func (p *printer) setComment(g *types.CommentGroup) {
	if g == nil || !p.useNodeComments {
		return
	}
	if p.comments == nil {
		// initialize p.comments lazily
		p.comments = make([]*types.CommentGroup, 1)
...
func (p *printer) identList(list []*types.Ident, indent bool) {
	// convert into an expression list so we can re-use exprList formatting
	xlist := make([]types.Expr, len(list))
	for i, x := range list {
		xlist[i] = x
	}
...
// expressions.
//
// TODO(gri) Consider rewriting this to be independent of []types.Expr
//           so that we can use the algorithm for any kind of list
//           (e.g., pass list via a channel over which to range).
func (p *printer) exprList(prev0 token.Pos, list []types.Expr, depth int, mode exprListMode, next0 token.Pos, isIncomplete bool) {
	if len(list) == 0 {
		if isIncomplete {
			prev := p.posFor(prev0)
			next := p.posFor(next0)
	...
	for i, x := range list {
		...
		pair, isPair := x.(*types.KeyValueExpr)
...
func (p *printer) parameters(fields *types.FieldList) {
...
func (p *printer) signature(params, result *types.FieldList) {
	if params != nil {
		p.parameters(params)
	} else {
...
func identListSize(list []*types.Ident, maxSize int) (size int) {
	for i, x := range list {
		if i > 0 {
			size += len(", ")
		}
...
func (p *printer) isOneLineFieldList(list []*types.Field) bool {
	if len(list) != 1 {
...
func (p *printer) setLineComment(text string) {
	p.setComment(&types.CommentGroup{List: []*types.Comment{{Slash: token.NoPos, Text: text}}})
}

func (p *printer) fieldList(fields *types.FieldList, isStruct, isIncomplete bool) {
	...
	if !hasComments && srcIsOneLine {
		...
		if len(list) == 0 {
			...
			if isStruct {
				...
				if ftyp, isFtyp := f.Type.(*types.FuncType); isFtyp {
					// method
					p.expr(f.Names[0])
...
func walkBinary(e *types.BinaryExpr) (has4, has5 bool, maxProblem int) {
	...
	case *types.BinaryExpr:
		if l.Op.Precedence() < e.Op.Precedence() {
			...
			// pretend this is an *types.ParenExpr and do nothing.
			break
		}
...
func cutoff(e *types.BinaryExpr, depth int) int {
...
func diffPrec(expr types.Expr, prec int) int {
	x, ok := expr.(*types.BinaryExpr)
...
//	   and 4 (never use spaces) in Compact mode.
//
func (p *printer) binaryExpr(x *types.BinaryExpr, prec1, cutoff, depth int) {
	prec := x.Op.Precedence()
	if prec < prec1 {
		// parenthesis needed
		// Note: The parser inserts an types.ParenExpr node; thus this case
...
func isBinary(expr types.Expr) bool {
	_, ok := expr.(*types.BinaryExpr)
	return ok
}

func (p *printer) expr1(expr types.Expr, prec1, depth int) {
	p.print(expr.Pos())
	...
	case *types.SliceExpr:
		...
		p.expr1(x.X, token.HighestPrec, 1)
		p.print(x.Lbrack, token.LBRACK)
		indices := []types.Expr{x.Low, x.High}
		if x.Max != nil {
	...
	case *types.CallExpr:
		...
		if _, ok := x.Fun.(*types.FuncType); ok {
...
func (p *printer) possibleSelectorExpr(expr types.Expr, prec1, depth int) bool {
	if x, ok := expr.(*types.SelectorExpr); ok {
...
// selectorExpr handles an *types.SelectorExpr node and returns whether x spans
// multiple lines.
func (p *printer) selectorExpr(x *types.SelectorExpr, depth int, isMethod bool) bool {
	p.expr1(x.X, token.HighestPrec, depth)
...
func (p *printer) expr0(x types.Expr, depth int) {
	p.expr1(x, token.LowestPrec, depth)
}

func (p *printer) expr(x types.Expr) {
	const depth = 1
	p.expr1(x, token.LowestPrec, depth)
}
...
func (p *printer) stmtList(list []types.Stmt, nindent int, nextIsRBrace bool) {
	if nindent > 0 {
		p.print(indent)
	}
	var line int
	...
	for _, s := range list {
		...
		if _, isEmpty := s.(*types.EmptyStmt); !isEmpty {
...
// block prints an *types.BlockStmt; it always spans at least two lines.
func (p *printer) block(b *types.BlockStmt, nindent int) {
...
func stripParens(x types.Expr) types.Expr {
	if px, strip := x.(*types.ParenExpr); strip {
		// parentheses must not be stripped if there are any
		// unparenthesized composite literals starting with
		// a type name
		types.Inspect(px.X, func(node types.Node) bool {
			switch x := node.(type) {
			case *types.ParenExpr:
...
func stripParensAlways(x types.Expr) types.Expr {
	if x, ok := x.(*types.ParenExpr); ok {
		return stripParensAlways(x.X)
	}
	return x
}

func (p *printer) controlClause(isForStmt bool, init types.Stmt, expr types.Expr, post types.Stmt) {
	p.print(blank)
	needsBlank := false
...
//
func (p *printer) indentList(list []types.Expr) bool {
...
func (p *printer) stmt(stmt types.Stmt, nextIsRBrace bool) {
	p.print(stmt.Pos())
	...
	case *types.LabeledStmt:
		...
		if e, isEmpty := s.Stmt.(*types.EmptyStmt); isEmpty {
			if !nextIsRBrace {
	...
	case *types.IfStmt:
		...
		if s.Else != nil {
			...
			switch s.Else.(type) {
			case *types.BlockStmt, *types.IfStmt:
...
//
func keepTypeColumn(specs []types.Spec) []bool {
...
func (p *printer) valueSpec(s *types.ValueSpec, keepType bool) {
	p.setComment(s.Doc)
...
func sanitizeImportPath(lit *types.BasicLit) *types.BasicLit {
	...
	return &types.BasicLit{ValuePos: lit.ValuePos, Kind: token.STRING, Value: s}
}
...
//
func (p *printer) spec(spec types.Spec, n int, doIndent bool) {
	switch s := spec.(type) {
...
func (p *printer) genDecl(d *types.GenDecl) {
	p.setComment(d.Doc)
		...
		if n := len(d.Specs); n > 0 {
			...
			if n > 1 && (d.Tok == token.CONST || d.Tok == token.VAR) {
				...
				for i, s := range d.Specs {
					...
					p.recordLine(&line)
					p.valueSpec(s.(*types.ValueSpec), keepType[i])
				}
			} else {
				var line int
				for i, s := range d.Specs {
...
//
func (p *printer) nodeSize(n types.Node, maxSize int) (size int) {
...
func (p *printer) numLines(n types.Node) int {
...
// bodySize is like nodeSize but it is specialized for *types.BlockStmt's.
func (p *printer) bodySize(b *types.BlockStmt, maxSize int) int {
	pos1 := b.Pos()
	pos2 := b.Rbrace
...
//
func (p *printer) funcBody(headerSize int, sep whiteSpace, b *types.BlockStmt) {
	if b == nil {
		return
	}
...
func (p *printer) funcDecl(d *types.FuncDecl) {
	p.setComment(d.Doc)
...
func (p *printer) decl(decl types.Decl) {
	switch d := decl.(type) {
	case *types.BadDecl:
...
func declToken(decl types.Decl) (tok token.Token) {
	tok = token.ILLEGAL
	switch d := decl.(type) {
	case *types.GenDecl:
		tok = d.Tok
...
func (p *printer) declList(list []types.Decl) {
	tok := token.ILLEGAL
	for _, d := range list {
		prev := tok
		tok = declToken(d)
...
func (p *printer) file(src *types.File) {
